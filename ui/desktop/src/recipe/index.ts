...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `codyno://recipe?config=${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!cleanLink.startsWith('codyno://recipe?config=')) {
      throw new Error('Invalid deeplink format. Expected: codyno://recipe?config=...');
    }

    const recipeEncoded = cleanLink.replace('codyno://recipe?config=', '');
