...
export const GOOSE_SERVE_EXITED_USER_MESSAGE =
  "This window's CodyNo backend stopped. Close this window and open a new chat to start a new backend. If this keeps happening, restart CodyNo.";
  ...
  create(result: GooseServeResult, secretKey: string): GooseServeLease {
    ...
    const markExited = ({
      ...
      if (logUnexpected && firstExit && !lease.cleanedUp) {
        this.logger.error('CodyNo ACP server exited unexpectedly', {
          code: lease.exitCode,
